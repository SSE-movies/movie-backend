...
def test_get_movies_with_multiple_filters(client):
    ...
    data = response.get_json()
    assert all(
        'inception' in movie['title'].lower() and
        movie['type'].lower() == 'movie' and
...
def test_invalid_page_number(client):
    ...
    assert response.status_code == 200  # Flask's default behavior is to use 1
    data = response.get_json()
    assert data['page'] == 1
...
def test_invalid_per_page_number(client):
    ...
    assert response.status_code == 200  # Flask's default behavior is to use 10
    data = response.get_json()
    assert data['per_page'] == 10 